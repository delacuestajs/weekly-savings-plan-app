...
<div class="max-w-5xl mx-auto bg-white rounded-lg shadow-md p-4 md:p-6">
    ...
    <form action="index.php?module=bag&action=update&id=<?= $bag['id'] ?>&return=<?= urlencode($_SERVER['HTTP_REFERER'] ?? 'index.php?module=bag') ?>" method="POST" enctype="multipart/form-data" class="max-w-lg">
        <?= Auth::csrfField() ?>
