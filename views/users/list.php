...
<div class="max-w-5xl mx-auto bg-white rounded-lg shadow-md p-4 md:p-6">
    ...
    <div class="flex flex-wrap gap-3 mb-4 md:mb-6">
        ...
        <a href="index.php" class="inline-block bg-gray-500 hover:bg-gray-600 text-white font-medium py-2 px-4 rounded-lg transition duration-200"><?= Locale::get('back_to_savings') ?></a>
    </div>
        ...
        <table class="w-full border-collapse">
            <thead>
                <tr class="bg-gray-100">
                    ...
                    <th class="p-3 text-left text-sm md:text-base font-semibold text-gray-700 hidden sm:table-cell"><?= Locale::get('nickname') ?></th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-50">
                        ...
                        <div class="relative group">
                            <?php if (!empty($row['picture'])): ?>
                                ...
                                <div class="absolute left-12 top-0 hidden group-hover:flex items-center gap-1 z-10 bg-white rounded-lg shadow-lg p-1">
                                    <button type="button" onclick="openModal('uploads/<?= htmlspecialchars($row['picture']) ?>', '<?= htmlspecialchars($row['firstname'] . ' ' . $row['lastname']) ?>', 'image')" class="p-1 hover:bg-gray-100 rounded" title="View">
                                        <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </button>
                                    <button type="button" onclick="openZoomModal('uploads/<?= htmlspecialchars($row['picture']) ?>', '<?= htmlspecialchars($row['firstname'] . ' ' . $row['lastname']) ?>')" class="p-1 hover:bg-gray-100 rounded" title="Zoom">
                    ...
                    <td class="p-3 text-sm md:text-base hidden sm:table-cell"><?= htmlspecialchars($row['nickname'] ?? '-') ?></td>
