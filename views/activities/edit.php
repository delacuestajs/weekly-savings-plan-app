...
<div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-4 md:p-6">
    <h1 class="text-2xl md:text-3xl font-bold text-gray-800 mb-4 md:mb-6"><?= Locale::get('edit_activity') ?></h1>
    
    <form action="index.php?module=activity&action=update&id=<?= $activity['id'] ?>" method="POST" class="space-y-4">
        <div>
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3" 
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"><?= htmlspecialchars($activity['description'] ?? '') ?></textarea>
        </div>
        ...
        <div>
            ...
            <input type="number" id="value" name="value" step="0.01" min="0" required 
                   value="<?= htmlspecialchars($activity['value']) ?>"
                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        </div>
